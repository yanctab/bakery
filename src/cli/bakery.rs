...
impl Bakery {
    ...
    pub fn bkry_exit(
        ...
        self.cli.debug(format!("Require Docker: {}", cmd_require_docker));
    ...
    pub fn unwrap_or_exit<T>(&self, 
        cmd: &str,
        cmd_require_docker: bool,
        force: bool,
        result: Result<T, BError>) -> T {
        result.unwrap_or_else(|err| {
            self.bkry_exit(&cmd.to_string(), cmd_require_docker, &err.to_string(), 1, force);
        })
    }
    ...
    pub fn bake(&self) {
         ...
         */
        self.cli.debug("Verify that we have access to the workspace.json".to_string());
        self.unwrap_or_exit::<()>(cmd_name, cmd_require_docker, true, cfg_handler.verify_ws());

        let settings: WsSettingsHandler =
            self.unwrap_or_exit::<WsSettingsHandler>(cmd_name, cmd_require_docker, true, cfg_handler.ws_settings());
        ...
        match cmd_result {
            Ok(command) => {
                ...
                self.cli.debug(format!("Build config: {}", config.build_data().name()));
                self.cli.debug(format!("Workspace dir: {:?}", settings.work_dir()));
                self.cli.debug(format!("Scripts dir: {:?}", settings.scripts_dir()));
                self.cli.debug(format!("Docker image: {}", settings.docker_image()));
                ...
                self.cli.debug(format!("Scripts dir: {:?}", workspace.settings().scripts_dir()));
                self.cli.debug(format!("Docker image: {}", workspace.settings().docker_image()));
            ...
            Err(err) => {
                self.bkry_exit(&cmd_name.to_string(), cmd_require_docker, &err.to_string(), 1, false);
            }
        }
