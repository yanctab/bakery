...
The build config is what describes building the product for Bakery. A typical build config would have a similar format as below
...
The config version is used to mark breaking changes to the build config format currently it is at version 5. If the format changes the version will be bumped and Bakery will alert that the build config format needs to be migrated to the new format.
...
Any context variable in the build config will be expanded when Bakery is parsing the build config. The context is a concept that is made up of two context variable type "built-in" variables and "config" variables. The "config" context variables are the ones defined in the context section of the build config while the "built-in" variables are coming from the `bkry` binary. The values of the "built-in" variables are either defined by the workspace.json or by a combination that the `bkry` binary will define in run-time. Currently the following "built-in" variables are available to be used in the build config
...
The machine value is mapped to the bitbake [MACHINE](https://docs.yoctoproject.org/singleindex.html#term-DISTRO) variable. This will be set in the local.conf file generated by Bakery in the build directory used by bitbake.
...
Bakery will take the localconf data and combine it with the other bitbake data available and create a local.conf in the builds dir defined in the [workspace conf](workspace-config.md#Customize). The local.conf is then used by bitbake when building the image. A [dry-run](sub-commands.mk#Dry-Run) can be executed using Bakery to generate the bitbake configurations files and skip the build.
...
Sometimes a task is needed but it should not be executed by default when not specifying a task and running a full build. For example a signing task that requires some additional resources like an HSM when signing so it should only be executed by a specific signing node then it can be disabled. It will then only be executed when the task is specifically specified in the `bkry` command using the task flag in the [build](sub-commands.md#Build).
...
from the command line when executing a `bkry build` command. For more information on what options to use run help
for the `bkry build` command.