...
14. Add support for locking workspace. When running bkry setup the workspace should be locked to the config. We will have to also add a new sub-command deej lock so it is possible to lock a workspace to a different config and we should be able to reset the build config also. This will need the meta-data support and workspace ID to work. We need to make sure that it is possible to call bkry build without a build config but if there is no workspace meta-data then it should be required.
